...
C_loss = function(C,AB,DY,Y_lag,M,rho){
  sum((DY - Y_lag%*%t(C))^2)/nrow(Y_lag) + 
    rho*sum((C-AB)^2)/2 + sum(M*C)
}
C_step_size = function(C,nabla_C,M,AB,DY,Y_lag,lambda,rho,
                      step_init=1,step_mult=0.5,step_max_iter = 1000){
  t = nrow(Y_lag)
  step_size = step_init/step_mult
  g_old = C_loss(C,AB,DY,Y_lag,M,rho)
  count = 0
  while(TRUE){
    count = count+1
    ...
    g_new = C_loss(C_new,AB,DY,Y_lag,M,rho)
...
C_prox = function(C,M,AB,DY,Y_lag,YY,DYY,lambda,rho,
  ...
  while(TRUE){
    ...
    if(step_size == "auto"){
      ...
      #Compute step size
      step_size_obj = C_step_size(C_old,nabla_C,M,AB,DY,Y_lag,
...
VECM_nuclear = function(C_init,A_init,B_init,M_init,Y,
                        lambda_nuclear,lambda_L2,lambda_L1,rho,
                        step_size = "auto",step_init=1,step_mult=0.5,
  ...
  while(TRUE){
    ...
    #Update C
    C_update = C_prox(C_old,M_old,AB_old,Y_diff,Y_lag,
    ...
    #Update A,B
    S = C_new + M_old/rho
    AB_update = AB_prox(A_old,B_old,S,rho,lambda_L2,lambda_L1,omegas,
...
VECM_NN_L2_L1 = VECM_nuclear(C_init = C_Johan,A_init = A_Johan,B_init = B_Johan,
                             ...
                             lambda_L2 = 0.01,lambda_L1 = 0.001,rho = 0.005,
                             step_size = "auto",step_init = 1,step_mult = 0.5,
                             step_max_iter = 100,max_iter = 1000,
                             ADMM_thresh = 1e-4,C_thresh = 1e-6,AB_thresh = 1e-4)
                         
VECM_NN_L2 = VECM_nuclear(C_init = C_Johan,A_init = A_Johan,B_init = B_Johan,
                          M_init = M_init,Y = Y,lambda_nuclear = 0.01,
                          lambda_L2 = 0.01,lambda_L1 = NULL,rho = 0.01,
                          step_size = "auto",step_init = 1,step_mult = 0.5,
                          step_max_iter = 100,max_iter = 1000,
                          ADMM_thresh = 1e-4,C_thresh = 1e-6,AB_thresh = 1e-4)
